...
from ..models import Orden, Sucursal, Examen, Resultados, Consulta
import json
...
@api_view(['POST'])
def ultimos_dias_sucursal(request):
    if request.method == 'POST':
        ...
        # Examenes
        cantHoy = len(Orden.objects.filter(sucursal=id_sucursal, consulta__fecha=hoy))
        cant7dias = len(Orden.objects.filter(sucursal=id_sucursal, consulta__fecha__gt=ultimos7dias))
        cant30dias = len(Orden.objects.filter(sucursal=id_sucursal, consulta__fecha__gt=ultimos30dias))
        examenes = {}
        ...
        cantHoy = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="LABORATORIO", consulta__fecha=hoy))
        cant7dias = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="LABORATORIO", consulta__fecha__gt=ultimos7dias))
        cant30dias = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="LABORATORIO", consulta__fecha__gt=ultimos30dias))
        examenes = {}
        examenes["hoy"] = cantHoy
        examenes["ultimos7dias"] = cant7dias
        examenes["ultimos30dias"] = cant30dias
        ...
        # Examenes Imagenes
        cantHoy = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="IMAGEN",
                                           consulta__fecha=hoy))
        cant7dias = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="IMAGEN", consulta__fecha__gt=ultimos7dias))
        cant30dias = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="IMAGEN", consulta__fecha__gt=ultimos30dias))
        examenes = {}
        examenes["hoy"] = cantHoy
        examenes["ultimos7dias"] = cant7dias
        examenes["ultimos30dias"] = cant30dias
        ...
        cantHoy = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="PROCEDIMIENTO",
                                           consulta__fecha=hoy))
        cant7dias = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="PROCEDIMIENTO", consulta__fecha__gt=ultimos7dias))
        cant30dias = len(Orden.objects.filter(sucursal=id_sucursal, examen__tipo_examen__categoria="PROCEDIMIENTO", consulta__fecha__gt=ultimos30dias))
        examenes = {}
        examenes["hoy"] = cantHoy
        examenes["ultimos7dias"] = cant7dias
        examenes["ultimos30dias"] = cant30dias
...
@permission_classes([IsAuthenticated])
@api_view(['POST'])
def examenes_frecuentes(request):
    if request.method == 'POST':
        payload = json.loads(request.body)
        id_sucursal = payload["id_sucursal"]
        examenes_frecuentes = []
        for orden in Orden.objects.filter(sucursal=id_sucursal).values('examen').annotate(total=Count('id')).order_by('-total')[:3]:
            examen_ele = {}
...
@ permission_classes([IsAuthenticated])
@ api_view(['POST'])
def facturacion_medicos(request):
    if request.method == 'POST':
        payload = json.loads(request.body)
        id_sucursal = payload["id_sucursal"]
        mes = payload["mes"]
        ano = payload["ano"]
        compensacion = Sucursal.objects.get(pk=id_sucursal).compensacion
        id_medico_actual = -1
        suma_laboratorio = 0
        suma_imagen = 0
